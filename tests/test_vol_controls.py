...
def test_vol_target_step_downscale_when_high_vol():
    ...
    w_new, scale = vol_target_step(w_prev, w_tgt, sigma_hat=0.20, sigma_tgt=0.10, clip=(0.5, 2.0))
...
def test_vol_target_step_upscale_when_low_vol():
    ...
    w_new, scale = vol_target_step(w_prev, w_tgt, sigma_hat=0.02, sigma_tgt=0.10, clip=(0.5, 2.0))
    # low sigma_hat -> scale would be 5.0, but clipped to 2.0 ⇒ Schritt ~0.8 L1, aber max 0.4 erreichbar -> voll zum Ziel
    assert scale == pytest.approx(2.0, 1e-6)
