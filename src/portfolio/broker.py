...
class PortfolioLite:
    def __init__(self, assets, initial_cash=1_000_000.0,
                 col_mark='adj_close_raw', col_ref='adj_open_raw',
                 col_spread='bid_ask_spread_corwin_schultz_raw',
                 ...
                 min_change_l1=0.05,  # No-Trade-Band (L1)
    ...
    def step(self, px_t: pd.DataFrame, px_t1: pd.DataFrame, w_target: pd.Series, cash_factor=None):

        # 1) Preise (Mark-to-Market nach Ausführung)
        adj_close_t = px_t[self.col_mark].astype(float).reindex(self.assets)
        adj_open_t_plus_1 = px_t1[self.col_ref].astype(float).reindex(self.assets)  # t+1 Adj_Open (Execution/Sizing)
        adj_close_t_plus_1 = px_t1[self.col_mark].astype(float).reindex(self.assets)  # t+1 Adj_Close (Bewertung)
        assert adj_open_t_plus_1.notna().any(), "p_ref (t+1 adj_open) hat nur NaNs – kein Handel möglich."
        assert adj_close_t_plus_1.notna().any(), "p_mark (t+1 adj_close) hat nur NaNs – Bewertung nicht möglich."

        if not getattr(self, "_did_openadj_check", False):
            if self.col_ref == "adj_open_raw" and self.col_mark == "adj_close_raw":
                print(f"[broker CHECK] NaNs: open_adj={int(adj_open_t_plus_1.isna().sum())}, adj_close={int(adj_close_t_plus_1.isna().sum())}")
        ...
        #print(f"adj_open_t_plus_1 {adj_open_t_plus_1}")
        #print(f"Cash {self.cash}")


        # 3) Portfolio-Wert vor Rebalance zum t+1-Preis
        Portfolio_Value_t = self.cash + float((self.shares * adj_close_t).sum())
        #print(f"Portfolio Value t {Portfolio_Value_t}")

        if not getattr(self, "_did_start_ping", False):
            print(f"[broker PING] A={len(self.assets)}, Ppre={Portfolio_Value_t:,.2f}, cash={self.cash:,.2f}")
            self._did_start_ping = True


        # 3a) Ziel-Stückzahlen @ t+1
        target_shares = (w * Portfolio_Value_t) / adj_close_t.replace(0.0, np.nan)
        ...
        exec_df = self.exec.plan_execution_series(
            q=q, adj_open=adj_open_t_plus_1, spread=spread,
        )
        # exec_df Spalten: ["q","adj_open","p_exec","notional_abs","spread_cost"]
        ...
        if self.cash - cash_delta < 0.0 and cash_delta > EPS:
            ...
            exec_df = self.exec.plan_execution_series(q=q, adj_open=adj_open_t_plus_1, spread=spread)
            fees_df = self.fees.apply_fees(exec_df, **self.fee_kwargs)
        ...
        self.value = self.cash + float((self.shares * adj_close_t_plus_1).sum())
        self.weights = (self.shares * adj_close_t_plus_1) / max(self.value, EPS)
